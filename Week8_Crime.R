...
murderMap$GunOwnerShipRate = murderMap$GunOwnership / murderMap$GunOwnership * 1000
...
# Redo the plot, using gunOwnership:
ggplot(murderMap, aes(x = long, y = lat, group = group, fill = murderMap$GunOwnership)) + geom_polygon(colour = "black") + scale_fill_gradient(low = "black", high = "red", guide = "legend", name = "gun ownership per 100k", limits = c(0.01,.597))
table(murderMap$GunOwnership,murderMap$State)
